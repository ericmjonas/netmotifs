...
class FastRelation(object):
    ...
    def __init__(self, domain_def, data, modeltype = None):
        ...
        for d in self.domains:
            self.domain_to_axispos[d] = tuple([n for n in self.axes if n == d])
        ...
        self.domain_groups = {}
        for d in self.domains:
            self.domain_groups[d] = np.ones(self.domains[d], dtype=np.int32)*NOT_ASSIGNED
        ...
        self.entity_to_dp = {domain_name: {i : [] for i in range(N)} for domain_name, N in self.domains.iteritems()}
    ...
    def _set_entity_group(self, domain_name, entity_pos, group_id):
        self.domain_groups[domain_name][entity_pos] = group_id

    def _get_entity_group(self, domain_name, entity_pos):
        return self.domain_groups[domain_name][entity_pos]

    def _get_dp_value(self, dp):
        return self.data[dp]
    ...
    def create_group(self, domain_name):
        ...
        for g in group_coords:
            self.components[k] = self.model.create_ss()
    ...
    def add_entity_to_group(self, domain_name, group_id, entity_pos):
        """
        """
        assert self._get_entity_group(domain_name, entity_pos) == NOT_ASSIGNED

        for dp in self._datapoints_for_entity(domain_name, entity_pos):
            value = self.get_data_value(dp)
            group_coords = self._get_dp_group_coords(dp)
            for axis_pos in self.get_axispos_for_domain(domain_name):
                group_coords[axis_pos] = group_id
            self.components[group_coords] = self.model.ss_add(self.components[group_coords], self.hps, data_value)
    ...
    def post_pred(self, domain_name, group_id, entity_pos):
        ...
        for dp in self._datapoints_for_entity(domain_name, entity_pos):
            ...
            for axis_pos in self.get_axispos_for_domain(domain_name):
                group_coords[axis_pos] = group_id
            score += self.model.post_pred(self.components[group_coords], self.hps, data_value)
